...
class AmapDataProvider(DataProvider):
    ...
    async def search_nearby(self, longitude, latitude, radius, categories):
        if self.mock:
            rows = [
                ...
                {
                    "source": "amap",
                    "provider_record_id": "mock-entertainment-1",
                    "name": "示例KTV",
                    "category": "娱乐",
                    "type_code": "080302",
                    "address": "候选点附近",
                    ...
                    "is_estimated": True,
                    "needs_verification": True,
                    "raw_data": {"mock": True, "query_group": "娱乐"},
                },
                {
                    "source": "amap",
            ...
            self.last_poi_diagnostics = {
                "provider": "amap",
                "mock": True,
                "queries": [{"group": group, "keywords": keywords, "raw_count": 1, "saved_count": 1} for group, keywords in self.poi_query_groups],
    ...
    def _category(poi: dict[str, Any]):
        ...
        groups = [
            ...
            ("敏感场所", ["政府机构", "政府机关", "医院"]),
            ("交通", ["地铁", "公交", "停车"]),
            ("娱乐", ["KTV", "量贩KTV", "歌厅", "酒吧", "台球", "电影院", "电影", "影城", "密室", "剧本杀", "桌游", "棋牌室"]),
            ("商业配套", ["商场", "餐饮", "美食", "奶茶", "便利店", "酒店"]),
            ("大学", ["大学", "学院"]),
            ("中职", ["中职", "职业中学", "职业学校"]),
            ("技校", ["技校", "技工学校"]),
            ("住宅小区", ["住宅小区", "小区", "居民区"]),
