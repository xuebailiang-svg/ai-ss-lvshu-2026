...
from migrations.helpers import column_is_nullable, index_exists, table_exists
...
def upgrade() -> None:
    if not table_exists("poi_enrichments"):
        op.create_table(
            "poi_enrichments",
    ...
    if not index_exists("poi_enrichments", "ix_poi_enrichments_poi_observation_id"):
        op.create_index("ix_poi_enrichments_poi_observation_id", "poi_enrichments", ["poi_observation_id"])
    if column_is_nullable("poi_observations", "longitude") is False:
        op.alter_column("poi_observations", "longitude", existing_type=sa.Float(), nullable=True)
    if column_is_nullable("poi_observations", "latitude") is False:
